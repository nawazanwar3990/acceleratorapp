<footer class="py-5" style="background-color: rgb(232, 228, 228);">
    <div class="container">
        <div class="row">
            <div class="col-md-4 col-md-4 col-xs-12 align-self-center footer-logo">
                <img src="{{ asset('uploads/header_logo.png') }}" alt="">
            </div>
            <div class="col-md-4 col-md-4 col-xs-12 nav-holder align-self-center">
                <div class="row footer-image">
                    <div class="col-6 col">
                        <a class="text-uppercase" style="color: #01023B;font-weight: bolder">About</a>
                        <img src="{{ asset('uploads/line-D1.png') }}" alt="">
                    </div>
                    <div class="col-6 border-start col right text-end">
                        <a class="text-uppercase" style="color: #01023B;font-weight: bolder">News</a>
                        <img class="image-right" src="{{ asset('uploads/line-D1.png') }}" alt="">
                    </div>
                    <div class="col-6 col">
                        <a class="text-uppercase" style="color: #01023B;font-weight: bolder">StartUps</a>
                        <img src="{{ asset('uploads/line-D1.png') }}" alt="">
                    </div>
                    <div class="col-6 border-start col right text-end">
                        <a class="text-uppercase" style="color: #01023B;font-weight: bolder">Contact</a>
                        <img class="image-right" src="{{ asset('uploads/line-D1.png') }}" alt="">
                    </div>
                </div>
            </div>
            <div class="col-lg-4 col-lg-4 mt-5 col-xs-12 align-self-center text-center">
                <h4 class="footer-text" style="color: #01023B;font-weight: bolder">Stay Updated with the latest news</h4>
                <div class="footer-buttons mt-3 mb-4">
                    <a class="btn btn-primary btn-xs mx-2 rounded-0" style="background-color: #01023B!important;">Subscribed</a>
                    <a class="btn btn-primary btn-xs btn-circle mx-2" style="background-color: #01023B!important;">
                        <i class="bx bxl-linkedin"></i>
                    </a>
                    <a class="btn btn-success btn-xs btn-circle mx-2" style="background-color: #01023B !important">
                        <i class="bx bxl-instagram"></i>
                    </a>
                    <a class="btn btn-primary btn-xs btn-circle mx-2" style="background-color: #01023B!important;">
                        <i class="bx bxl-twitter"></i>
                    </a>
                </div>
                <p class="text-muted fs-13">©{{ date('Y') }} Accelerator ALL Rights Reserved.</p>
            </div>
        </div>
    </div>
</footer>
