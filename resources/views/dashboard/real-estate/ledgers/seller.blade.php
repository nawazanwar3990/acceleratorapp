...
@section('content')
    <div class="row">
        <div class="col-12">
            @include('dashboard.real-estate.ledgers.components.seller-ledger-filters')
            <div class="card shadow-none pt-0" id="printArea">
                @include('components.General.form-list-header', ['print' => true])
                <div class="card-body" id="printArea">
                    <h3 class="text-dark card-title">{{ __('general.seller_ledger') }}
                        @if (request()->has('seller_id') && request()->get('seller_id') != '')
                            [ {{ $sellerName }} ]
                        @endif
                    </h3>
                        <table class="table table-striped border no-wrap table-hover" id="datatable" style="width: 100%;">
                            ...
                            <tbody>
                                @include('dashboard.real-estate.ledgers.components.seller-ledger-list')
                            </tbody>
                        </table>
                </div>
            </div>
