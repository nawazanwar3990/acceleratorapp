    ...
    <div class="row">
        <div class="col-12">
            <div class="card shadow-none pt-0">
                ...
                <div class="card-body">
                    {!! Form::model($model, ['url' =>route('dashboard.postal-dispatch.update', $model->id), 'method' => 'POST','files' => true,'id' =>'postal_dispatch_form', 'class' => 'solid-validation']) !!}
                    @method('PUT')
                    <x-updated-by-field />
                    <x-hidden-building-id />
                    @include('dashboard.real-estate.front-desk.postal-dispatch.fields', ['for' => 'edit'])
                    <x-buttons :update="true" :cancel="true" cancelRoute="dashboard.postal-dispatch.index"/>
                    {!! Form::close() !!}
                </div>
            </div>
        </div>
