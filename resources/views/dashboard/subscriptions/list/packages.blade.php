                                ...
                                <td class="text-center">
                                    @if(\App\Services\GeneralService::isExpireSubscription(\Carbon\Carbon::now(),$subscription->expire_date))
                                        <a class="btn btn-xs btn-warning mx-1"
                                           href="{{ route('dashboard.payment-receipts.index',['type'=>$subscription->subscribed->roles[0]->slug,'id'=>$subscription->id,'process'=>'renew']) }}">
                                            {{ trans('general.renew') }} <i
...
{{--@section('innerScript')
    <script>
        function apply_package_action(subscription_id, type) {
            Swal.fire({
                title: type + ' Package',
                html: `<?php echo Html::decode(Form::label('payment_type' ,__('general.payment_type').'<i class="text-danger">*</i>' ,['class'=>'form-label'])); ?><?php echo e(Form::select('payment_type',\App\Enum\PaymentTypeEnum::getTranslationKeys(),\App\Enum\PaymentTypeEnum::OFFLINE,['class'=>'form-control','id'=>'payment_type','placeholder'=>'Select Payment Type'])); ?>`,
                ...
                if (payment_type === '<?php echo e(\App\Enum\PaymentTypeEnum::OFFLINE); ?>') {
                    Swal.fire({
                        title: 'Payment Transaction Id From Receipt',
                        html: `<?php echo Html::decode(Form::label('transaction_id' ,__('general.transaction_id').'<i class="text-danger">*</i>' ,['class'=>'form-label'])); ?><?php echo e(Form::text('transaction_id',null,['class'=>'form-control','id'=>'transaction_id'])); ?>`,
                        ...
                        $.ajax({
                            url: "<?php echo e(route('dashboard.payments.store')); ?>",
                            method: 'POST',
                            data: data,
