...
class PostalDispatchRequest extends FormRequest
{
    /**
    ...
    {
        $model = PostalDispatch::create($this->all());;
        if ($model) {
    ...
    private function uploadMedia($model)
    {
        ...
        if (count($documents)) {
            foreach ($documents as $document) {
                ...
                $path = 'uploads/postal-dispatch/documents/' . $fileName;

                $document->move('uploads/postal-dispatch/documents', $fileName);

                Media::create([
                    'filename' => $path,
                    'record_id' => $model->id,
                    'record_type' => 'postal_dispatch_document',
    ...
    {
        $deleteData = Media::where('record_type','postal_dispatch_document')->where('record_id',$model->id)->delete();
        $documents = $this->file('documents', []);
