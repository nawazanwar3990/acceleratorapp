...
class DeviceMakeRequest extends FormRequest
{
    ...
    public function rules()
    {
        if (in_array($this->method(), [
            ...
            if ($this->method() == MethodEnum::PUT) {
                ...
                return [
                    'name' => ['required', Rule::unique(TableEnum::DEVICE_MAKES)->ignore($model_id)]
                ];
            } else {
                return [
                    'name' => ['required', Rule::unique(TableEnum::DEVICE_MAKES)]
                ];
            }
        } else {
            return [];
    ...
    {
        $model = DeviceMake::create($this->all());
        if ($model) {
