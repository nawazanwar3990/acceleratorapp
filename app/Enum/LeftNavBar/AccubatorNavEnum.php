...
class AccubatorNavEnum extends AbstractEnum
{
    ...
    {
        $routes = array(
            self::BUILDING =>/* ($params) ? route('website.buildings.index', $params) : route('website.buildings.index')*/null,
            self::FLOOR =>/* ($params) ? route('website.floors.index', $params) : route('website.floors.index')*/null,
            self::OFFICE => /*($params) ? route('website.offices.index', $params) : route('website.offices.index')*/ null
        );
        if (!is_null($key) && array_key_exists($key, $routes)) {
            return $routes[$key];
        } else {
